        ...
        for n in range(1,repeat+1):
            for mode,width,height in [('desktop',1440,1100),('mobile',390,844)]:
                ...
                try:
                    ...
                    diagnostic=dict(error=str(error),page_errors=errors,release_commit=health.get('release_commit'),
                        body=page.locator('body').inner_text(),
                        assets=page.evaluate("performance.getEntriesByType('resource').map(e=>({name:e.name,start:e.startTime,end:e.responseEnd}))"))
                    if sid:
                        response=page.request.get(base+'/api/route/journeys/'+sid)
                        diagnostic['server_status']=response.status
                        diagnostic['server_state']=response.json()
                    (output/f'{mode}-{n}-failure.json').write_text(json.dumps(diagnostic,ensure_ascii=False,indent=2))
                    raise
                finally:
                    context.close()
        browser.close()
