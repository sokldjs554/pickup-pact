...
def seed_legacy_posting(order: str, event: str, reason: str, amount: int, unit: str) -> None:
    fingerprint = hashlib.sha256(f"{order}|{reason}|{amount}|{unit}".encode()).hexdigest()
    with psycopg.connect(DSN) as connection:
        connection.execute(
            "insert into ledger_batches(event_id,semantic_fingerprint,aggregate_id,reason) values(%s,%s,%s,%s)",
            (event, fingerprint, order, reason),
        )
...
def unattributed_legacy_reversal_blocks_new_reversal() -> None:
    for reason, reverse, amount, unit in (
        ...
        seed_legacy_posting(order, order + "-legacy-reversal", reverse, amount, unit)
...
def legacy_projection_without_provenance_is_not_overwritten() -> None:
    ...
    with psycopg.connect(DSN) as connection:
        connection.execute(
            ...
            "values(%s,'CANCELLED',true,false,false,0,0,0,%s)", (order, "1" * 64),
        )
    events = [dict(
    ...
    with psycopg.connect(DSN) as connection:
        row = connection.execute(
            "select status,canonical_hash,source_event_count from commitment_projection where aggregate_id=%s",
            (order,),
        ).fetchone()
        assert row == ("CANCELLED", "1" * 64, 0), row
