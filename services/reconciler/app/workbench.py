...
def create_router(database_path):
    ...
    def perform(fn, *args):
        try:
            ...
            return JSONResponse({"detail": "sample session or plan not found"}, status_code=404)
    ...
    def page():
        return HTMLResponse(
            Path(__file__).with_name("repair_workbench.html").read_text(encoding="utf-8"),
            headers={
    ...
    @router.post(
        "/api/repair-lab/sessions",
        status_code=201,
        response_model=SessionView,
        responses=MUTATION_ERRORS,
    )
    def new_session(body: NewSession):
        return perform(store.create, body.case)

    @router.get(
        "/api/repair-lab/sessions/{session_id}",
        response_model=SessionView,
        responses={404: {"description": "Sample not found"}},
    )
    def session(session_id: UUID):
        return perform(store.get, str(session_id))

    @router.post(
        "/api/repair-lab/sessions/{session_id}/simulate",
        response_model=SessionView,
        responses=MUTATION_ERRORS,
    )
    def simulate(session_id: UUID, body: Simulate):
        return perform(store.simulate, str(session_id), body.action)

    @router.post(
        "/api/repair-lab/sessions/{session_id}/plans",
        status_code=201,
        response_model=PlanView,
        responses=MUTATION_ERRORS,
    )
    def preview(session_id: UUID, body: PlanRequest):
        return perform(store.preview, str(session_id), body.version)

    @router.post(
        "/api/repair-lab/sessions/{session_id}/plans/{plan_id}/approve",
...
def create_app(database_path):
    ...
    def home():
        return RedirectResponse("/repair-lab")
