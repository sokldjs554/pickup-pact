...
## 2026-09-26 추가 보완의 근거

상품·참여 범위와 비용 부담은 `agreement.py`에서 견적에 묶고 수령 시 모의 수취액의 균형을 검사한다. 실제 고객 인터뷰나 점주 계약이 있다는 의미는 아니다. `response_models.py`는 핵심 주문/금액/복구 응답을 명시한다. 열린 이벤트 payload와 실서비스 운영 공백은 유지된다.

비교는 네 방식이다. 새 자리 확보 후 재주문에도 같은 안전 절차를 적용하므로 거절/자리 경쟁에서 주문 보호는 동률이다. 이를 숨기지 않으며 같은 주문 ID 유지가 실제 만족도를 높였다고 추정하지 않는다. 저장소 초기화를 줄인 성능 비교는 내구성 설정을 유지한 로컬/CI 짝비교이고 실제 서비스 지연은 별도로 측정해야 한다. 최신 검증 상태는 새 HEAD의 CI와 배포 결과로만 확인한다.

지원서 작성 전 사용자에게 새 화면과 변경 근거를 보여준다. [전체 범위](decision-review-2026-09-26.md).
