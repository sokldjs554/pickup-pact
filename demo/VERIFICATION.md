...
- `ci`: repository guardrails, Python/JVM tests, contracts, promise-admission evidence reproduction, Docker builds, PostgreSQL execution-plan capture, and Terraform validation. The Kotlin commitment `CONFIRMED → PICKED_UP` transition and `PickupClaimed` event must stay aligned with the customer pickup-code flow.
