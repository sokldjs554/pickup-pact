...
첫 화면에서는 주문 이벤트나 Kafka, ledger 같은 기술 용어를 먼저 보여주지 않습니다. 방문자는 **`직접 확인해보기` 버튼 한 번**으로 아래 흐름을 실제 데모 API에서 자동 실행합니다.

1. **정상 주문 준비** — 9,000원 주문과 12:30 픽업 약속을 정상 확정합니다.
2. **취소 발생** — 고객은 주문을 취소하지만 취소 메시지는 서버에 52초 늦게 도착합니다.
3. **잘못된 지급** — 그 사이 점주 정산 9,000원과 고객 포인트 90P가 남습니다.
4. **문제 탐지** — 실제 발생 순서를 다시 확인해 취소 뒤에 남은 정산과 포인트를 잘못된 처리로 판단합니다.
5. **사용자가 복구 실행** — 방문자는 `문제 복구하기`만 직접 눌러 **정산 0원 · 포인트 0P**로 돌아오는 결과를 확인합니다.

즉, 일반 사용자에게는 **문제 → 자동 확인 → 복구 전후 결과**만 먼저 보여주고, 구현 세부사항은 `백엔드 구현 보기`를 눌렀을 때만 별도 전문가 모드에 표시합니다.
