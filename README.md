...
- 가상 참여 매장 사이에서만 360mL·아이스·우유·디카페인 조건을 확인하고 옮깁니다. 맛·브랜드가 같다고 가정하지 않습니다.
- 바뀌는 메뉴·할인과 고객/플랫폼/매장 부담 조건을 견적에 묶고, 수령 때 모의 수취액을 한 번만 기록합니다.
...
- **6개 상황의 실행 비교:** 기존 매장 유지·취소 후 재주문·새 자리 확보 후 재주문·확인 후 같은 주문 변경을 같은 명령 처리기로 실행합니다. 정상 처리나 응답 재시도에서 같았던 결과도 남깁니다. 차이는 새 매장 거절·자리 선점 시 원래 주문이 남는지입니다. 실행 요청 수는 클릭 수나 소요 시간을 측정한 값이 아닙니다.
