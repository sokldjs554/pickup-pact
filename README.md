...
## 공개 데모: 제품 화면과 개발자 화면을 분리

공개 URL의 기본 화면은 관리 콘솔이 아니라 **주문 보호/복구 제품 화면**입니다. 첫 방문에서는 사이드바, Kafka, ledger, event ID 같은 구현 세부사항을 숨기고, 실제 서비스처럼 주문 보호 상태와 복구 결과를 먼저 보여줍니다.

### 주문 복구 센터

첫 화면의 `복구 시나리오 체험하기`를 누르면 실제 데모 API가 아래 흐름을 자동 실행합니다.

1. **주문 확정** — 9,000원 주문과 12:30 픽업을 정상 상태로 준비합니다.
2. **취소 발생** — 고객 취소가 먼저 발생하지만 취소 정보는 서버에 52초 늦게 도착합니다.
3. **잘못된 지급** — 그 사이 점주 정산 9,000원과 고객 포인트 90P가 남습니다.
4. **문제 감지** — 제품 화면에 주의 상태와 잘못된 금액을 보여줍니다.
5. **복구 실행** — 사용자가 `잘못된 처리 되돌리기`를 누르면 **정산 0원 · 포인트 0P**로 정상화됩니다.

일반 모드에는 제품 헤더, 주문 보호 미리보기, 주문 복구 센터, 복구 전/후 비교만 노출됩니다. 백엔드 운영 콘솔은 `개발자 구현 보기` 또는 `개발자 화면 열기`를 선택했을 때만 표시됩니다.
